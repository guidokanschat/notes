...
\begin{proof}
  ...
  \slideref{Theorem}{qr-existence} using the property
  \begin{gather}
    \spann{\vq_1,\dots,\vq_j} = \spann{\vv,\mata\vv,\dots,\mata^{j-1}\vv}
  \end{gather}
  from \slideref{Lemma}{qr-columns}. Comparing this
...
\begin{Theorem}{arnoldi-linear-system}
  ...
  \begin{align}
    \vx^{(m+k)} &\in \vx^{(k)} + \krylov_m(\vr^{(k)},\mata)\\
    \vb-\mata \vx^{(m+k)} &\perp \krylov_m(\vr^{(k)},\mata),
  \end{align}
  ...
  and let
  \begin{gather}
    \vx^{(m+k)} = \vx^{(k)} + \matq_m \vy.
  \end{gather}
\end{Theorem}

\begin{proof}
  Let $\vv$ be the update vector such that
  $\vx^{(m+k)} = \vx^{(k)} + \vv$ and note that this implies
  $\vv=\matq_m\vy$.

  The orthogonality condition implies for all basis vectors $\vq_i\in\krylov_m(\vr^{(k)},\mata)$
  \begin{gather}
    0 = \scal(\vb-\mata \vx^{(m+k)},\vq_i) = \scal(\vr^{(k)} - \mata\vv,\vq_i).
  \end{gather}
  Hence,
  \begin{gather}
    \vq_i^* \mata\vv = \vq_i^*\vr^{(k)},\qquad i=1,\dots,m,
  \end{gather}
  or in matrix notation
  \begin{gather}
    \matq_m^*\mata\vv = \matq_m^* \vr^{(k)}.
  \end{gather}
  Since $\matq_m$ is orthogonal and $\vr^{(k)} = \beta\vq_1$, the
...
\begin{Theorem}{arnoldi-linear-residual}
  ...
  \begin{gather}
    \vb-\mata\vx^{(m+k)} = -h_{m+1,m} \ve_m^T \vy_m \vq_{m+1},
  \end{gather}
  and therefore
  \begin{gather}
    \norm{\vb-\mata\vx^{(m+k)}}_2 = h_{m+1,m} \abs{\ve_m^T \vy_m}.
  \end{gather}  
\end{Theorem}

\begin{proof}
  See also~\cite[Proposition 6.7]{Saad00} We use the definition of the
  residual of $\vx^{(m+k)}$ and the representation of
  \slideref{Theorem}{arnoldi-projection} to obtain
  \begin{align}
    \vr^{(m+k)} = b-\mata\vx^{(m+k)}
    &= b- \mata\left(\vx^{(k)} + \matq_m\vy_m\right)\\
    &=\vr^{(k)} - \mata\matq_m\vy_m\\
...
\begin{Lemma}{lanczos-orthogonality}
  For the sequences of vectors $\vx_m$, $\vr_m = \vb-\mata\vx^{(m)}$ and $\vg_m$
...
\begin{proof}
  ...
  $\matq_m$. Furthermore, it is symmetric, positive definite, the
  latter assuming $\mata$ is invertible.  Furthermore,
  $\vr^{(0)} = \vb-\mata\vx^{(0)} = \beta\matq_{m+1}\ve_1$, such that
  \begin{gather}
    \matq_m^T\mata^T\vr^{(0)} = \beta\overline\matH_m \ve_1.
  \end{gather}
...
\end{todo}
\begin{intro}
  The GMRES algorithm uses the Arnoldi process in order to compute a QR factorization of the 
\end{intro}
...
\begin{remark}
  ...
  \begin{itemize}
  \item Restarted GMRES simply deletes the whole basis $\matq_m$ and the matrix $\overline\matH_m$ every $k$-th step starts fresh.
