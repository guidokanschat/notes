...
\begin{proof}
  ...
  \begin{gather}
    \mu_k = \max_{\substack{W \subset V\\\dim W = k}} \min_{\vx\in V} R_\mata(\vx)
    \le \max_{\substack{W \subset \R^n\\\dim W = k}} \min_{\vx\in V} R_\mata(\vx)
    = \lambda_k.
  \end{gather}
\end{proof}
...
\begin{Lemma}{ritz-rayleigh-estimate}
  ...
  $R_\mata(PVK \vu)$ admits the estimate
  \begin{gather}
...
\begin{Theorem}{ritz-convergence}
  Let $\esp{1,\dots,m}$ be the space spanned by the dominant $m$
  eigenvectors and let $\matp$ be the orthogonal projector onto
  it. Let $S_k$ be the subspace spanned by the basis $\matx^{(k)}$.
