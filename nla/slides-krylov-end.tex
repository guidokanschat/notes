\subsubsection{GMRES}
\frame{\subsubtoc}
\frame {\footnotesize\input {blocks/Theorem-arnoldi-linear-system.tex}
\footnotesize\input {blocks/Theorem-arnoldi-linear-residual.tex}}
\frame {\input {blocks/Theorem-normal-equations.tex}}
\frame {\input {blocks/Lemma-oblique-normal.tex}}
\frame {\input {blocks/Lemma-gmres-projected.tex}
\footnotesize\input {blocks/Theorem-arnoldi-projection.tex}}

\frame {\input {blocks/Algorithm-gmres.tex}}
\frame {\input {blocks/Theorem-gmres-projection.tex}
  \pause
  \input {blocks/Lemma-gmres-breakdown.tex}}
\frame {\input {blocks/Theorem-gmres-optimality.tex}}
\frame {\input {blocks/Corollary-gmres-estimate.tex}}

\frame {\input {blocks/Lemma-qr-Arnoldi-1.tex}}
\frame {\input {blocks/Algorithm-arnoldi-householder.tex}}

\frame {\input {blocks/Algorithm-gmres-restart.tex}}
\frame {\input {blocks/Algorithm-gmres-truncated.tex}}
\frame {\input {blocks/Theorem-gmres-pos-def.tex}}

\subsubsection{Lanczos Biorthogonalization}
\frame{\subsubtoc}
\frame {\input {blocks/Lemma-projection-basis.tex}}
\frame {\input {blocks/Algorithm-bi-lanczos.tex}}
\frame {\input {blocks/Theorem-bi-lanczos-projection.tex}}

\subsubsection{Preconditioning}
\frame{\subsubtoc}
\frame {\input {blocks/Definition-preconditioned-system.tex}}
\frame {\input {blocks/Definition-preconditioner.tex}}
\frame {\input {blocks/Lemma-preconditioner-symmetry}
  \pause
  \setbeamercolor{block title}{fg=black!70!white,bg=iwrred!40!white}
  \setbeamercolor{block body}{fg=black!60!white, bg=black!5!white}
  \footnotesize
  \input {blocks/Algorithm-lanczos.tex}}
\frame {\input {blocks/Algorithm-pcg.tex}}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "slides"
%%% End:
