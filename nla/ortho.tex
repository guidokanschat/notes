...
\begin{todo}
  GM:
  We later need \(Q_w^* x = \alpha e_1\), Instead of \(Q_w x = \alpha e_1\). \\
  For Givens it is already defined as such.
\end{todo}
...
\begin{todo}
  GM:
  Should the proof include, that the choice of \(\phi\) is restricted to a factor of \(-1\)? Maybe as a remark?
\end{todo}
\begin{proof}
