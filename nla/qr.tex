...
\begin{remark}
  At this point, we are missing two items to write a functioning algorithm.
  \begin{enumerate}
  \item The shifted QR iteration should be in implicit form.
  \item We need a way to make sure that the shifted algorithm is only
    applied to unreduced matrices.
  \end{enumerate}
  Both questions will be addressed in the upcoming section on real matrices.
\end{remark}
