...
\begin{Theorem*}{implicit-Q}{Implicit Q Theorem}
  ...
  $\vv_j = e^{i\phi_j}\vq_j$ and $\abs{h_{j,j-1}} = \abs{g_{j,j-1}}$
...
\begin{proof}
  ...
  \end{gather}
  For $j=1$, we obtain from the orthogonality of $\matq$ and $\matv$
  and the parallelity of $\vq_1$ and $\vv_1$ that
  $\vw_1 = e^{i\phi} \ve_1$ for some argument $\phi$.
\end{proof}
...
\begin{todo}
  What was the final result on Hermitian matrices?
\end{todo}
