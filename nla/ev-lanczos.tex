...
\begin{Lemma}{lanczos-angle}
  ...
  $\theta_i^{(m)}$ between the eigenspace $\esp{i}$ and the Krylov space
...
\begin{proof}
  See also~\cite[Lemma 6.1]{Saad11}. For arbitrary $\vw\in\krylov_m$,
  let $\vw = q(\mata)\vv_1$ with a polynomial $q\in \P_{m-1}$. We have
  the orthogonal decomposition
  \begin{gather}
    \vw = q(\mata)\vv_1 = q(\mata) P_i \vv_1 + q(\mata) (\id-P_i)\vv_1.
  \end{gather}
  Let $\vu_i = P_i\vw = q(\mata) P_i\vv_1$ be the orthogonal
  projection of $\vw$ into the eigenspace $\esp{i}$ and hence the
  closest eigenvector to $\vw$. Then, for the angle $\theta$ between
  $\vw$ and $\vu_i$ there holds
  \begin{gather}
    \tan \theta = \frac{\norm{q(\mata)(\id-P_i)\vv_1}}{\norm{q(\mata)P_i\vv_1}}
    = \frac{\norm{q(\mata)\vy_i}}{\abs{q(\lambda_i)}}
    \frac{\norm{(\id-P_i)\vv_1}}{\norm{P_i\vv_1}}.
  \end{gather}
  For the second equality, we have assumed $(\id-P_i)\vv_1\neq0$,
  which we may safely do, because $(\id-P_i)\vv_1=0$ implies
  $\vv_1\in\esp{i}$, which is an invariant subspace and hence
  $\theta=0$.

  Now let $p(\lambda) = \nicefrac{q(\lambda)}{q(\lambda_i)}$ to obtain
  \begin{gather}
    \tan \theta = \norm{p(\mata)\vy_i} \tan \angle(\vv_1,\esp{i}).
  \end{gather}
\end{proof}
...
\begin{Theorem}{lanczos-eigenvalue-estimate}
  The approximate eigenvalues $\lambda_i^{(m)}$ after $m$ steps of the Lanczos method satisfy the estimate
  \begin{gather}
