...
\begin{proof}
  ...
  projection. We present here the version which is also used in
  functional analysis. To this end, we define for arbitrary nonzero $\vw\in K$
  the function
  \begin{gather}
  ...
  \begin{align}
    F(t) &= t^2\norm{\vw}_{\mata}^2 + 2t \scal(\tilde\vx-\vx,\vw)
    + \norm{\tilde\vx-\vx}_{\mata}^2,\\
    F'(t) &= 2t\norm{\vw}_{\mata}^2 + 2 \scal(\tilde\vx-\vx,\vw)_{\mata},\\
  ...
  \begin{gather}
    F'(0) = 2 \scal(\tilde\vx-\vx,\vw)_{\mata} = 0,
  \end{gather}
...
\begin{proof}
  ...
  \slideref{Theorem}{projection-orthogonal-optimal}.
  \begin{todo}
    continue
  \end{todo}
\end{proof}
