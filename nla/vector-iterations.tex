...
\begin{Definition}{shifted-matrix-polynomial}{Shifted Matrix Polynomial}
  ...
  The \define{matrix polynomial} of degree \(m\), with shift parameter \(\sigma_1, \ldots, \sigma_m\) is defined by
  \[ \mathbf{p(A)} := \left( A - \sigma_1 \id \right) \dots \left( A - \sigma_m\right).\]
\end{Definition}
