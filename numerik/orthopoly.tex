...
\begin{proof}
  Das Skalarprodukt ist wohldefiniert, das gilt also ebenso für die Abbildung.
  Wir müssen nun die Normeigenschaften nachrechnen.\\
  Seien $u,v \in V$.\\
  1)Nichtnegativität
  \begin{gather}
    \norm {u} = \sqrt{\scal(u,u)} \ge 0 \quad\text{da\,postitv-definit}
    \end{gather}
  2) Definitheit
  \begin{gather}
    0 = \norm{u} =\sqrt{\scal(u,u)}\\
    \Rightarrow 0 = \scal(u,u)\\
    \Rightarrow u = 0
  \end{gather}
  3)Homogenität
  \begin{gather}
  ...
  \end{gather}
  Daraus folgt $\norm{u+v} \le \norm{u}+\norm{v}$\\
  Die Ungleichung, die in 4) verwendet wird ist die
  Cauchy-Schwartz-Ungleichung und wird noch unter $\cite{bcs}$ behandelt.
...
\begin{proof}
  Zunächst zeigen wir nur die Ungleichung:
  Für $v=0_V$ klar. Hier ist anzumerken, dass $0_V$ aus dem Vektorraum stammt
  und nicht zwangsläufig die Zahl Null ist.\\
  
   Seien $v,u \in V$ und $ v \neq 0, u\neq 0$. Für $\mu, \lambda \in \R$
  gilt in Bilinearform folgendes  
  \begin{gather}
  ...
  \begin{gather}
    ...
    \Rightarrow \scal(u,v) \le \norm u \cdot \norm v
  \end{gather}

  Nun bleibt die Äquivalenz für die Gleichheit zu zeigen.\\
  Für $v=0$ ist die Äquivalenz klar. Sei also $v\neq 0$ und $u\neq 0$. \\\\
  $\glqq \Rightarrow \grqq$ Seien u,v linear abhängig.Das heißt $u=av$.\\
  Dann gilt
  \begin{gather}
    \abs{\scal(u,v)} = \abs{a \scal(v,v)}
    = \abs{a} \cdot \norm v \cdot \norm v
    = \norm{av} \cdot \norm v =\norm u \cdot \norm v  
  \end{gather}
  
  $ \glqq \Leftarrow  \grqq$ Es gilt die Gleichheit.\\
  $\mu = \scal(u,v)\neq 0 $ sowie
  $\lambda = \scal(v,v)\neq 0$. Dann erhält man
  \begin{gather}
   \scal(u,v) = \norm u \cdot \norm v\\
   \scal(u,v)^{2} = \scal(u,u) \scal(v,v)\\
   0 = \scal(v,v) \scal(u,u) -2\scal(u,v)^{2}+\scal(u,v)^{2}
  \end{gather}
  Setze nun $\mu = -\scal(u,v)$ und multipliziere mit $\scal(v,v)$
  \begin{gather}
    0 =\scal(v,v)^{2} \scal(u,u)+2\mu \scal(u,v)\scal(v,v) +\mu^{2}\scal(v,v)
  \end{gather}
  Setze $\lambda = \scal(v,v)$ und erhalte
  \begin{gather}
    0=\lambda^{2} \scal(u,u)+2 \mu \lambda \scal(u,v) +\mu^{2} \scal(v,v)\\
    0=\scal(\lambda+\mu v,\lambda u+\mu v)
  \end{gtaher}
  
  Da es sich um eine Bilinearform handelt folgt nun
