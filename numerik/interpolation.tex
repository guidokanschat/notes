...
\begin{Definition}{newton-basis}
  Als \define{Newton-Basis} der Hermite-Interpolation bezeichnen wir
  die Polynome
  \begin{gather}
...
\begin{Definition}{div-diff-1}
  ...
  Lagrange-Interpoaltionsaufgabe bezeichnen wir die rekursiv
  definierten Werte
  \begin{align}
    [x_i]f
    &= f_i \\
...
\begin{proof}
  ...
  \end{gather}
  Ferner gilt für die Polynome
  \begin{gather}
    t_{ij} = \frac{(x-x_i)^j}{j!}
    \qquad\text{dass}\qquad
    \frac{d^\ell t_{k\ell}}{d x^\ell} (x_k) = \delta_{ik}\delta_{j\ell}.
  \end{gather}
  Damit ist die Matrix der Interpolationsbedingungen angewandt auf die
  doppelt indizierte Basis $h_{ij}(x) = t_{ij}(x) q_i(x)$ diagonal.
\end{proof}
...
\begin{Notation}{interpolation-ascending}
  ...
  \end{gather}
  $k$-fach wiederholte Stützstellen bedeuten, dass dort nicht nur der
  der Funktionswert, sondern auch die ersten $k-1$ Ableitungen
  interpoloert werden. Damit haben wir f''ur die Interpolation in
  $\P_n$ immer eine Folge von $n+1$ Stützstellen.
\end{Notation}
