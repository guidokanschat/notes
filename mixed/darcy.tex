...
\begin{Example}{rt-simplex}
  ...
  \begin{center}
    \begin{tabular}{c@{\hspace{.2\textwidth}}c}
      \includegraphics[width=.3\textwidth]{./fig/rt0-tri.tikz}
      &
      \includegraphics[width=.3\textwidth]{./fig/p0-p.tikz}
      \\[5mm]
      \includegraphics[width=.3\textwidth]{./fig/rt1-tri.tikz}
      &
...
\begin{Definition}{canonical-interpolation}
  ...
  such that for any $f\in C^\infty(\cell)$ there holds
  \begin{gather}
    \nodal_i I_h(f) = \nodal_i(f)
    \qquad
    i=1,\dots,n,
...
\begin{proof}
  ...
  \end{gather*}
  Then, tha left hand side of the equation above becomes
...
\begin{remark}
  ...
  any for $s>0$. Thus, if the domain allows for an inf-sup condition
  of the form
  \begin{gather*}
