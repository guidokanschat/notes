...
\begin{Notation}{musical-isomorphisms}
  ...
  differential geometry, we denote them as \define{musical
    isomorphism}s
  \begin{xalignat}2
    ...
    &\phi&\mapsto \phi^\flat.
  \end{xalignat}
  \index{sharp@$\sharp$}\index{flat@$\flat$}
\end{Notation}

\begin{Problem}{riesz-h10}
  Let $V=H^1_0(\domain)$ with $\domain=(0,1)$ be equipped with the
  inner product
  \begin{gather}
...
\begin{Problem}{polar-orthogonal}
  Show that $\polar{W} = I_\sharp \ortho{W}$ and $\dualpolar{U} = I_\flat \ortho{U}$.
\end{Problem}
