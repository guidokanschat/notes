...
\begin{Theorem}{orthogonal-complement}
  ...
  orthogonal complement. Then, $V = W \oplus W^\perp$ if and only if
  $W$ is closed.
\end{Theorem}

\begin{proof}
  We show that $W^\perp = \overline{W}^\perp$. Once this is
  established, the ``only if'' follows by the fact, that if $W$ is not
