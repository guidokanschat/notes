...
\begin{Definition}{legendre-higher-dimension}
  ...
  \begin{gather}
    ...
    = \prod_{m=1}^d \frac{2j_m+1}2 \int_{-1}^{1} \plegendre_{j_m} \plegendre_{i_m}(x_m) \dx_m,
  \end{gather}
...
\begin{Definition}{moment-basis-1d}
  The shape function space $\P_k$ on the reference element $\refcell = [-1,1]$ in one dimension can be split into
  \begin{gather}
    \label{eq:stokes:35}
    \P_k^0 \oplus \overline{\P_k},
  \end{gather}
  where $\P_k^0 = \P_k \cap H^1_0(\refcell)$. We choose an
...
\begin{Problem}{moment-dofs}
  ...
  \item Find an expression for the basis functions (up to
    normalization) in $\P_k^0$ in one dimension terms of Legendre polynomials
    (Wikipedia?)
