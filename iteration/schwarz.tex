...
\begin{proof}
  ...
  \begin{align*}
    \scal(B^{-1}v,v)
    &= \sum_{j=1}^J \scal(B^{-1} v, A_j^{-1} A_j v_j)
    = \sum_{j=1}^J \scal(\Pi_j B^{-1} v, A_j^{-1} A_j v_j) \\
    &\le \sum_{j=1}^J \sqrt{\scal(\Pi_j B^{-1} v, A_j^{-1}\Pi_j B^{-1} v)}
    \sqrt{\scal(A_j v_j, A_j^{-1}A_j v_j)} \\
    & \le \sqrt{\sum_{j=1}^J \scal(\Pi_j B^{-1} v, A_j^{-1}\Pi_j
      B^{-1} v) }
    \sqrt{\sum_{j=1}^J \scal(A_j v_j,A_j^{-1}A_j v_j)} \\
    &= \sqrt{\scal(B^{-1} v, {\sum A_j^{-1}\Pi_j} B^{-1}v)}
    \sqrt{\sum_{j=1}^J \scal(A_j v_j, v_j)} \\
    &= \sqrt{\scal(B^{-1}v,v)} \sqrt{\sum_{j=1}^J a(v_j, v_j)},
  \end{align*}
  yields for arbitrary decompositions
  \begin{gather}
    \label{eq:schwarz:17}
    \scal(B^{-1}v,v) \le \sum_{j=1}^J a(v_j, v_j),
  \end{gather}
  ...
  \begin{align*}
    ...
    &= \sum_{j=1}^J \scal(A_j A_j^{-1} \Pi_j B^{-1} v, A_j^{-1} \Pi_j
    B^{-1} v) \\
    &= \sum_{j=1}^J \scal(\Pi_j B^{-1} v, A_j^{-1} \Pi_j B^{-1} v) \\
...
\begin{note}
  ...
  In order to estimate the condition number $\Lambda(BA)/\lambda(BA)$ of a Schwarz
  ...
  order to find a bound for $\Lambda(BA)$, we have to find an estimate of
  the form
  \begin{gather}
    \label{eq:schwarz:23}
  ...
  order to bound $1/\lambda(BA)$, we need an estimate of the form
  \begin{gather}
    \label{eq:schwarz:24}
...
\begin{definition}
  ...
  \end{xalignat}
  Similarly, we can define bartitions of unity in $H^1(\Omega)$ or
...
\begin{example}
  ...
  following procedure:
  \begin{enumerate}
  \item Begin with a nonoverlapping subdivision $\{\Omega_j^0\}$,
...
\begin{proof}
  ...
  is as much orthogonality as we get). Let therefore $o_{jk}$ be a
  ...
  \begin{align*}
    ...
    &= \sum_{j,k=1}^J o_{jk}  a(v_j,v_k) \\
    & \le \sum_{j,k=1}^J \sqrt{o_{jk} a(v_j, v_j)} \sqrt{o_{jk} a(v_k,
      v_k)} \\
    & \le \sum_{j,k=1}^J o_{jk} a(v_j, v_j) \\
...
\begin{proof}
  ...
  by Clement or the one by Schott and Zhang. For a given function
...
\begin{remark}
  ...
  \begin{gather*}
    \kappa(B_{\text{TLS}} A_h) \lesssim \left(1+\frac H h\right)^2.
  \end{gather*}
