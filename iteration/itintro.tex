...
\begin{definition}
  We call the bilinear form $a(.,.)$ \define{symmetric}, if
  holds
  \begin{gather*}
    a(u,v) = a(v,u) \qquad \forall u,v \in V.
  \end{gather*}
  It is \define{elliptic}, if for there is a positive number $\gamma$
  such that
  \begin{gather*}
...
\begin{definition}
  For positive definite self adjoint operators
  \define{Rayleigh quotient}
  \begin{gather}
    \label{eq:richardson:8}
    \Lambda(A) = \max_{u\in V_n}\frac{a(u,u)}{\norm{u}_V^2},
    \qquad
    \lambda(A) = \min_{u\in V_n}\frac{a(u,u)}{\norm{u}_V^2}.
  \end{gather}
\end{definition}
