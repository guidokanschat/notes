...
\begin{proof}
	$y'$ is the interpolation polynomial of degree $s-1$ in the
	interpolation points $c_1, \dots, c_s$. Since \dots, there holds
	\begin{gather*}
	...
	We now write
	\begin{gather*}
		y(t) - u(t) = \int_0 ^t y'(\tau) - u'(\tau) \d \tau \le
		\int_0 ^t h^s \cdot c \max_{t \in [t_0, t_1]} |u^{(s+1)} (t)|
		\d \tau = ch^s t \le ch^{h+1}.
	\end{gather*}
