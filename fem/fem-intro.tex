...
\begin{proof}
  \begin{figure}
    ...
    \node (mid) at (\mid,\mid) {$\hat{x_0}$};
    \coordinate (x1) at (\scale*1,0);
    \node[right = \scale*0.01 of x1] (x1name) {$\hat{x_1}$};
    \coordinate (x2) at (0,\scale*1);
    \node[above = \scale*0.01 of x2] (x2name) {$\hat{x_2}$};
    \coordinate (x3) at (0,0);
    \node[left = \scale*0.01 of x3] (x3name) {$\hat{x_3}$};
    \coordinate (xc) at (\outermid,\outermid);
    \node[above = \scale*0.01 of xc] (xcname) {$\hat{x_c}$};
    ...
    \draw (mid) circle (\mid);
    \draw (mid) --  (\mid,0);
    \node[below right= \scale*0.001 and \scale*0.0001 of mid] (rho) {$\hat{\rho}$};
    
    % circumcircle
    \draw (xc) circle (\outerrad);
    \draw (xc) -- (\outermid+\outerrad,\outermid);
    \node[above right = \scale*0.01 and \scale*0.25 of xc] (R) {$\hat{R}$};
    \end{tikzpicture}
  \end{figure}

  Let us define $S_{\reference{\rho}}\coloneqq \{\refv\in\R^d ~\mid~ \abs{\refv}=\reference{\rho} \}$.
  For $\refv\in S_{\reference{\rho}}$ there holds
  $\refx\coloneqq \reference{x_0} + \refv\in \overline{\refcell}$
  and $x=\matb\refx + \vb = \matb(\reference{x_0}+\refv)+\vb$.
  Additionally, we have $x_0 = \matb \reference{x_0} + \vb$
  which yields
  \begin{align*}
    \abs{x-x_0} = \abs{\matb\refv}\leq 2R.
  \end{align*}
  Now consider the operator norm of $\matb$
  \begin{align*}
    \norm{\matb} = \sup_{\refv\in\R^d} \frac{\abs{\matb\refv}}{\abs{\refv}}
    = \sup_{\abs{\refv}=1} \abs{\matb\refv}
    = \sup_{\refv\in S_{\reference{\rho}}} \frac{\abs{\matb\refv}}{\abs{\refv}}
    \leq \frac{2R}{\reference{\rho}}.
  \end{align*}
  $\reference{\rho}$ is a constant only depending on $\refcell$ but
  not on $\cell$. Hence, we get $\norm{\matb}\leq cR$ with $c$ depending
  on $\reference{\rho}$.

  The same argument can be applied for the second estimate.
  We now define $S_\rho\coloneqq \{v\in\R^d~\mid~\abs{v}=\rho \}$.
  For $v\in S_\rho$ there holds $x\coloneqq x_0+v\in\overline{\cell}$
  and let $\refx=\matb^{-1}x-b,~\reference{x_0}=\matb^{-1} x_0 - \vb$ which yields
  \begin{align*}
    \abs{\refx - \reference{x_0}} = \abs{\matb^{-1} v}\leq 2\reference{R}.
  \end{align*}
  Analogous to the first estimate we obtain
  \begin{align*}
    \norm{\matb^{-1}} = \sup_{v\in S_\rho} \frac{\abs{\matb^{-1} v}}{\abs{v}}
