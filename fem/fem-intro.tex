...
\begin{Lemma}{shape-function-basis}
  Let $\nodal^1,\dots,\nodal^{n_T}$ be the node functionals of the
  finite element and let $\phi_1,\dots,\phi_{n_T}$ be an arbitrary
  ...
  and only if $\matg$ is invertible. Furthermore, the shape function
  basis $\{p_k\}$ can be computed by
  \begin{gather}
    \begin{pmatrix}
      p_1(\vx)\\\vdots\\p_{n_T}(\vx)
    \end{pmatrix}
    = \matg^{-1}
    \begin{pmatrix}
      \phi_1(\vx)\\\vdots\\\phi_{n_T}(\vx)
    \end{pmatrix}.
  \end{gather}
\end{Lemma}
