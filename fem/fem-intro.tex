...
\begin{Definition}{conforming}
  A finite element space $V_{\mesh}$ is called \define{conforming}
...
\begin{Definition}{fe-space}
  ...
  We choose the \define{nodal basis} which has one basis function for
...
\begin{Definition}{tensor-product-polynomials}
  ...
  $q_i\in\P_k$ are called \define{elementary tensor} or \define{rank
    one tensor}.
\end{Definition}
...
\begin{Example}{Bogner-Fox-Schmit}
...
\begin{remark}
  The norm of $\matb$ is equal to its greatest singular value, while
  the norm of $\matb^{-1}$ is the reciprocal of the smallest singular
  value of $\matb$. Furthermore, the determinant is the product of the
  singular values, since
  \begin{gather}
    \prod_{i=1^d} \sigma_i(\matb)^2 = \det(\matb^T\matb) = \det \matb^T \det\matb
    = (\det\matb)^2.
  \end{gather}
  Hence, we can completely characterize the estimates in
  \slideref{Lemma}{mapped-norms-affine} by singular values.
\end{remark}

\begin{intro}
  The preceding estimate for the scaling of integrals and norms holds
  for affine mappings only. Even there, we face the problem that for
  the reference simplex the fraction between inradius and circumradius
  converges to zero as the dimension grows.

  We need a finer tool though, which is less dependent on simple
  geometric properties, but on understanding the character of the
  mapping.
\end{intro}
...
\begin{example}
  ...
  following three steps, using as $h_\cell$ the length of a characteristic
  edge of $\cell$.
  \begin{enumerate}
  \item Choose $\Phi_O$ as the rigid body movement which maps the
    vertices of the characteristic edge to the origin and the point $(h_\cell,0)$, respectively. This mapping
    has the structure
    \begin{gather*}
  ...
  \item Choose the scaling
    ...
    \end{gather*}
    such that the end points of the characteristic edge of the
    resulting cell $\cell_S$ are the origin and the point $(1,0)$,
    respectively..
  \item Warp the cell $\cell_S$ into the reference cell $\refcell$ by
    the mapping $\Phi^{-1}_W$. This operation leaves the two fixed
    points untouched. For triangles, it is the uniquely defined linear
    transformation mapping the vertex not on the longest edge to
    $(0,1)$. For quadrilaterals, it is a bilinear transformation. For
    higher order mappings, it could be anything.
  \end{enumerate}
...
\begin{Lemma*}{scaling-1}{Scaling lemma}
  ...
  \begin{gather}
    \begin{split}
      ...
      \\
      \norm*{(\nabla\Phi_W(\refvx))^{-1}} \le m_\cell^{-1} ,
      \\
...
\begin{proof}
  ...
  \slideref{Lemma}{nodal-continuity} holds. Hence, the function values
...
\begin{Algorithm}{inhomogeneous-essential-boundary}
