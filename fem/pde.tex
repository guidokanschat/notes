...
\begin{proof}
  Let us first consider test functions $v\in H^1_0(\domain)$.
  Then we can add a boundary term to \eqref{eq:neumann-weak-formulation}:
  \begin{align}
    \label{eq:neumann-weak-with-boundary}
    \int_\domain fv \dvx
    = \int_\domain \nabla u \cdot \nabla v \dvx
  ...
  \begin{align*}
    - \int_\domain \Delta uv \dvx = \int_\domain fv \dvx.
  \end{align*}
  This gives us $-\Delta u = f$ in $\domain$ for $u\in H^1_0(\domain)$.
  Subtracting equation \eqref{eq:neumann-weak-with-boundary} from
  \eqref{eq:neumann-weak-formulation} we obtain
  \begin{align*}
    \int_{\d\domain} \d_n u v \ds = 0.
  \end{align*}
  As we actually wanted to solve in $H^1(\domain)$ we will test
  the boundary term with functions $v\in H^1(\domain)$.
  Hence, we have $\partial_n u = 0$ on $\d\domain$ and
  $-\Delta u = f$ in $\domain$ for $u\in H^1(\domain)$.  
\end{proof}
...
\begin{proof}
  ...
  \begin{gather*}
    \begin{aligned}
      \int_\domain fv \dx &= \int_\domain -\Delta u v\dx = \int_\domain \nabla u \cdot \nabla v\dx - \int_{\d\domain} \d_n u v \ds \\
      &= \int_\domain \nabla u \cdot \nabla v \dx - \underbrace{\int_{\Gamma_D} \d_n u v \ds}_{=0} - \int_{\Gamma_R} \d_n u \ds \\
      &= \int_\domain \nabla u \cdot \nabla v \dx - \int_{\Gamma_R} (g - \alpha u) v\ds.
      \end{aligned}
  \end{gather*}
...
\begin{remark}
  In \slideref{Example}{mixed-bc-weak} essential boundary conditions are shown
  and \slideref{Lemma}{neumann-weak} can be understood as an example for natural
  boundary conditions.
\end{remark}
